...
namespace VerTrans
{
    public class fc
    {
        ...
        [DllImport("gdi32")]
        public static extern int AddFontResource(string lpFileName); //安裝字體
        [DllImport("gdi32")]
        ...
        public static string[] cboA = new string[] { @"\\10.40.40.127\Cosmos_patch\對外區", @"\\10.40.140.211\Work\2259\WorkingFolder" };
        ...
        //TP07-----------------------------------------------------------------------------------------------------
        public static string PGNamePath = Mydocument + @"\VerTrans\PGName.ini";

        //GP3 暫時
        //public static string[] cboA = new string[] { @"\\10.40.40.127\gp3\POS_Update\對外區" };

        public static string user = "";
        //public static string RCVerInfoIni = @"\\10.40.140.211\WorkingFolder\ResourceCoust\POSVerNode.ini";
        //public static string RCVerInfoIniTmp = @"\\10.40.140.211\WorkingFolder\ResourceCoust\POSVerNode_Tmp.ini";
        //public static string RCInc = @"\\10.40.140.211\WorkingFolder\ResourceCoust\ResourceConst";
        //public static string RCDic = @"\\10.40.140.211\WorkingFolder\ResourceCoust\";
        ...
        public static void WriteConfigIni1(string xSetupName, string[] xValue, string[] tp06, string VerNo, string VerNoDefault, 
            string CurrenInfo, string SetupVerNo, string tp04_Path, 
            string tp7_UserName, string tp06_PW_PasswordChar,
            bool chkUpdatePI, string[] LoginS, bool IsPOSAutoLogin,bool IsClearPI032)
        {
            try
            {
                ...
                //----檢查節點是否存在                
                string[] Snode = new string[] { "ProductVersion", "SetupName", "SetupPath", "Handle", "Account", "ToolPath", "NoCopyFiles", "TP06", "Library2003", "VerNo", "CurrenInfo", "SetupVerNo", "TP04", "TP07", "UpdatePI", "AutoLogin", "Options" };
                ...
                string[] tmp5K = source.Configs["TP06"].GetKeys();
                string[] tmp5V = source.Configs["TP06"].GetValues();
                string[] tmp6K = source.Configs["AutoLogin"].GetKeys();
                string[] tmp6V = source.Configs["AutoLogin"].GetValues();
                //ProductVersion---------------------------------------------
                ...
                if (tmp5K.Length > 0)
                {
                    for (int i = 0; i < tmp5K.Length; i++)
                    {
                        source.Configs["TP06"].Set(tmp5K[i], tmp5V[i]);
                    }
                    source.Configs["TP06"].Set("ID", source.Configs["TP06"].GetString("ID", ""));
                    source.Configs["TP06"].Set("PW", source.Configs["TP06"].GetString("PW", ""));
                    source.Configs["TP06"].Set("PWChar", source.Configs["TP06"].GetString("PWChar", "●"));                    
                ...
                else
                {
                    ...
                    source.Configs["TP06"].Set("PWChar", tp06_PW_PasswordChar);  
                }
                //VerNo
                if (VerNo == "")
                {
                ...
                //TP07
                source.Configs["TP07"].Set("UserName", tp7_UserName);  
                //UpdatePI
                /*if (chkUpdatePI)
                    source.Configs["UpdatePI"].Set("AutoUpdate", chkUpdatePI);  
                else*/
                    source.Configs["UpdatePI"].Set("AutoUpdate", chkUpdatePI);
                ...
                if (tmp6K.Length > 0)
                {
                    for (int i = 0; i < tmp6K.Length; i++)
                    {
                        source.Configs["AutoLogin"].Set(tmp6K[i], tmp6V[i]);
                    }
                }
                else
                {
                    source.Configs["AutoLogin"].Set("LoginID", LoginS[0]);
                    source.Configs["AutoLogin"].Set("LoginPW", LoginS[1]);
                }
                //IsPOSAuto
                source.Configs["Options"].Set("IsPOSAutoLogin", IsPOSAutoLogin);
    
                //20160301 add  by Erric for 20160301001  ↓
                //IsClearPI032
                source.Configs["Options"].Set("IsClearPI032", IsClearPI032);
                //20160301 add  by Erric for 20160301001  ↑
                source.Save();
        ...
        public static void PrevInstance()
        {
            if (Process.GetProcessesByName(Process.GetCurrentProcess().ProcessName).Length > 1)
            {
                fc.ShowBoxMessage("程式已開啟!","錯誤");
                System.Environment.Exit(0);
            }
        }
        ...
        public static string EmptyAtEnd(string xvalue, int xLen)
        {
            string mResult = "";
            if (xvalue.Length >= xLen)
            {
            ...
            else
            {
                ...
                mResult = xvalue + mResult;
                return mResult;
            }
        }
        public static string EmptyAtFirst(string xvalue, int xLen)
        {
            string mResult = "";
            if (xvalue.Length >= xLen)
            {
        ...
        public static bool ConnNetUse(string ip,string id,string pw)
        {
            ...
            //網路磁碟路徑
            string UncPath = @"\\" + ip + @"\Cosmos_patch";
            //網域
            string Domain = "workflowerp";
            //帳號
            string User = id;
            //密碼
            string Passowrd = pw;
            ...
            if (unc.IsConnect)
            {
                //MessageBox.Show("已連線到 " + UncPath);
                return true;
            }
            else
            {
                WriteLog(UncPath + " 連線失敗\r\n" + "錯誤代碼 " + unc.LastError.ToString(), true);
                return false;
            }
        ...
        public static bool ShowBoxInput(string s, out string str)
        {
            BoxInput b = new BoxInput();
            b.SetMsg = s;
        ...
        public static DialogResult ShowBoxMessage(string s,int sec)
        {
            BoxMessage b = new BoxMessage();
            b.SetSeconds = sec;
            b.SetMsg = s;
        ...
        public static bool isFontExist(string FontFileName)
        {
            ...
            {
                if (fontFamilies[j].Name.ToUpper() == FontFileName.ToUpper())
                {
                    return true;
                }
            }
        ...
        public static bool installFont(byte[] files,string FontFileName, string FontName)
        {
            ...
            try
            {
                FilePath = @"\\10.40.140.211\Work\4940\Tools\VerTrans\YaHei.Consolas.1.12.ttf";
                FontFamily[] fontFamilies;
                InstalledFontCollection installedFontCollection = new InstalledFontCollection();
                ...
                RemoveFontResourceA(FilePath);
                if (!isFontExist)
                {
                    /*File.Copy(System.Windows.Forms.Application.StartupPath +
                         "\\font\\" + FontFileName, FilePath); //font是程序目录下放字体的文件夹
                    FilePath =DirVerTrans + @"\" + FontFileName+".ttf";*/
                    //File.WriteAllBytes(FilePath, files);

                    Ret = AddFontResource(FilePath);
                    if (Ret != 0) ShowBoxMessage("[ " + FontName + " ]字形安装成功！", "訊息");
                    File.Delete(FilePath);
                }
            }
            catch (Exception ex)
            {
                ShowBoxMessage("[ " + FontName + " ]字型安装失敗！原因：" + ex.Message,"錯誤");
                return false;
            }
            return true;
        }
        public static void AddItemToCbo(ComboBox c)
        {
            string s = c.Text;
            List<string> tmp = new List<string>();
            tmp.Add(s);
        ...
        public static void INSERTPOSPI(string connstr,List<string> xL,int xtype)
        {
 
        }
        public static void UPDATEPOSPI()
        {
